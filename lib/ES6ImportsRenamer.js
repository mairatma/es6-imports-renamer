...
function ES6ImportsRenamer(config, callback) {
	...
	this._normalizeFn = config.normalizeFn || this._defaultNormalizeFn;
...
/**
 * Default function used for normalizing a path name to be renamed.
 * @param {string} path
 * @param {string} parentName
 * @param {!function} callback
 * @protected
 */
ES6ImportsRenamer.prototype._defaultNormalizeFn = function(path, parentName, callback) {
	callback(path);
}
...
ES6ImportsRenamer.prototype._mapImport = function(node, parentName) {
	...
	return new Promise(function(resolve, reject) {
		self._normalizeFn(originalPath, parentName, function(normalized) {
			self._renameFn(normalized, function(renamed) {
